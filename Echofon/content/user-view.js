...
function addToList(elem)
{
  var list = elem.list;
  var req = new TwitterClient(account, this);
  var method = 'lists/members';
  var callback = '';
  req.list = list;
  if (elem.subscribed) {
    method += '/destroy';
    callback = 'list_deleted';
  }
  else {
  	method += '/create';
  	callback = 'list_added';
  }
  req.post(method, {list_id:list.id,user_id:gUser.id}, callback);
}
...
function setUser(user)
{
  ...
  if(user.description && $('user-description-multiline').textContent.length == 0) {
