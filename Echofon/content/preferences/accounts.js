var gAccountsPane = {
	init : function() {
		...
		while (list.firstChild)
			list.removeChild(menu.firstChild);
	...
	onAddAccount : function() {
		this.oauthWindow = window.openDialog("chrome://echofon/content/OAuth.xul", "Echofon:OAuth", "chrome,dialog=yes,titlebar,toolbar,centerscreen,resizable=no,dependent=yes,width=800,height=660");
	...
	promptPasswordDialog : function(user, pass, msg) {
		...
		while (1) {
			var result = prompt.promptUsernameAndPassword(window, "Echofon", msg, user, pass, "", {
				value : false
			});
			if (!result)
				return false;
	...
	onMuteSetting : function() {
		var win = window.openDialog("chrome://echofon/content/preferences/mute.xul", "Echofon:mute", "chrome,dialog=yes,titlebar,toolbar,centerscreen,resizable=no,dependent=yes");
	...
	onRemoveAccount : function() {
		...
		if (user_id == EchofonCommon.pref().getCharPref("activeUserId")) {
			if (list.firstChild) {
				EchofonCommon.notify("changeAccount", {
					user_id : list.firstChild.value
				});
			} else {
				EchofonCommon.notify("logout");
			}
		}
		EchofonCommon.notify("removeAccount", {
			user_id : user_id
		});
