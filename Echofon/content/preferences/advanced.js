var gAdvancedPane = {
	...
	initKeyboardShortcut : function() {
		...
		var pref = Components.classes['@mozilla.org/preferences-service;1'].getService(Components.interfaces.nsIPrefService).getBranch("ui.key.");
		...
		case 224:
			...
			break;
		default:
			this.platformKeys.accel = (window.navigator.platform.search("Mac") == 0 ? this.platformKeys.meta : this.platformKeys.control);
		}
	...
	clearCache : function() {
		...
		var flags = prompt.BUTTON_POS_0 * prompt.BUTTON_TITLE_IS_STRING + prompt.BUTTON_POS_1 * prompt.BUTTON_TITLE_IS_STRING;
		var check = {
			value : false
		};
		var apptype = EchofonCommon.isXULRunner() ? "Echofon" : "Firefox";
		var button = prompt.confirmEx(window, "Echofon", EchofonCommon.getFormattedString("WarnClearCache", [ apptype ]), flags, EchofonCommon.getString("RestartNow"), EchofonCommon.getString("RestartLater"), "", null, check);
		if (button == 0) {
