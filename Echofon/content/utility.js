...
if (typeof EchofonCommon == 'undefined') {
	...
	var EchofonCommon = {
		...
		pref : function() {
			if (!this._pref) {
				this._pref = Components.classes['@mozilla.org/preferences-service;1'].getService(Components.interfaces.nsIPrefService).getBranch("extensions.twitternotifier.");
			}
			return this._pref;
		},
		...
		createTweetCell : function(uid, tweet, highlighted) {
			if (document.getElementById("echofon-status-" + tweet.type + "-" + tweet.id))
				return null;
			...
			try {
				...
				var time = EchofonCommon.createAnchorText(EchofonCommon.twitterURL(user.screen_name + "/statuses/" + permalink), label, "link", "echofon-status-timestamp");
				...
				if (msg.in_reply_to_status_id && msg.in_reply_to_screen_name) {
					if (elem.appMode == "window" || msg.type == 'user-timeline') {
						var reply = EchofonCommon.createAnchorText(EchofonCommon.twitterURL(msg.in_reply_to_screen_name + "/statuses/" + msg.in_reply_to_status_id), EchofonCommon.getFormattedString("inReplyToInline", [ msg.in_reply_to_screen_name ]), "tweet-popup", "echofon-source-link echofon-source-link-left-padding");
						...
						var reply = EchofonCommon.createAnchorWithElements(EchofonCommon.twitterURL(msg.in_reply_to_screen_name + "/statuses/" + msg.in_reply_to_status_id), [ icon, text ], "tweet-popup", "echofon-source-link");
						info.appendChild(reply);
					}
				}
				/*
				 * if (msg.metadata && msg.metadata.result_type == "popular") {
				 * if (msg.metadata.recent_retweets) { var recentRT =
				 * document.createElement("description");
				 * recentRT.appendChild(document.createTextNode(", retweeted " +
				 * msg.metadata.recent_retweets + " times")); recentRT.className =
				 * "echofon-top-tweet-retweets"; info.appendChild(recentRT); } }
				 */
		...
		findStatusElement : function(tweet_id) {
			var ret = [];
			var types = [ 'home', 'mentions', 'lists', 'search', 'user-timeline', 'favorites' ];
			for ( var i in types) {
		...
		distanceOfTimeInWords : function(minutes, shortFormat) {
			...
			if (minutes < 1)
				...
				index = (minutes == 1 ? 'MinuteAgo' : 'MinutesAgo');
			else if (minutes < 90)
				index = 'AboutOneHourAgo';
		...
		gotoUser : function() {
			...
			var result = prompts.prompt(null, EchofonCommon.getString("goToUser"), EchofonCommon.getString("enterScreenName"), input, null, check);
		...
		reloadTimeline : function() {
			EchofonCommon.notify("refresh", {
				user_id : this.pref().getCharPref("activeUserId")
			});
		},
		...
		openURL : function(url, event, newTab, forceToSetActive) {
			...
			if (event) {
				if (event.button != 0)
					newTab = true;
				if (this.isAccelKeyPressed(event))
					newTab = true;
				if (event.shiftKey)
		...
		openPreferences : function() {
			...
			if (win) {
				...
				window.openDialog("chrome://echofon/content/preferences/preferences.xul", "_blank", "chrome,dialog=yes,titlebar,toolbar,centerscreen,resizable=no,dependent=yes");
			}
		},

		openPurchase : function() {
			window.openDialog("chrome://echofon/content/purchase.xul", "Echofon:Purchase", "chrome,dialog=yes,titlebar,toolbar,centerscreen,resizable=yes,dependent=yes,width=800,height=640");
		},
		...
		startOAuth : function(screen_name, callback) {
			var win = window.openDialog("chrome://echofon/content/OAuth.xul?" + screen_name, "Echofon:OAuth", "chrome,dialog=yes,titlebar,toolbar,centerscreen,resizable=no,dependent=yes,width=800,height=660");
