  ...
  playSound: function() {
    if (EchofonCommon.pref().getBoolPref("sound")) {
      try {
        ...
        tempLocalFile.initWithPath(EchofonCommon.pref().getCharPref("soundFile"));

        const ioService = Components.classes['@mozilla.org/network/io-service;1'].getService(Components.interfaces.nsIIOService);
        this._notifySound.src = ioService.newFileURI(tempLocalFile).spec;
        this._notifySound.volume = EchofonCommon.pref().getIntPref("soundVolume") / 100;
        this._notifySound.play();
      }
      catch (e) {
      }
    }
