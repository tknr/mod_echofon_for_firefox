...
const STREAM_ADDRESS = "https://userstream.twitter.com/1.1/user.json";
...
TwitterClient.prototype = {
  ...
  stream: function(query) {
    ...
    var authStr = TwitterClient.buildOAuthHeader(this._user, "GET", STREAM_ADDRESS, param);
    ...
    request.setURL(STREAM_ADDRESS + '?' + params.join('&'));
  ...
  onLoad: function(req, param) {
    try {
      EchofonUtils.debug(req.callback + " (" + this._user.screen_name + ") - " + req.status + " " + req.statusText +
                 " (Rate limit: " + req.getResponseHeader("X-Rate-Limit-Remaining") + "/"
                 + req.getResponseHeader("X-Rate-Limit-Limit") + ")");
    }
    catch (e) {}
