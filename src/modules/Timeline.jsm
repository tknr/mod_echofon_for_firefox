...
Cu.import("resource://echofon/MuteWord.jsm");
...
TimelineLoader.prototype = {
  ...
  lists_statuses: function(resp, req, context) {
    if (resp) {
      ...
      if (resp.length == 0) {
        this.notifyObservers("removeLoadMoreCell", LIST_TIMELINE);
      }
    }
  ...
  retrieveTweets : function(obj, type, context) {
    ...
    for (var i = obj.length - 1; i >= 0; --i) {
      ...
      if (MuteWord.instance().checkMuteWord(this.token.user_id, obj[i])) continue;
  ...
  searchTweets: function(query, max_id, no_stream) {
    ...
    var req = new TwitterClient(this.token, this);
    req.query = query;
    var params = {'include_entities':true, 'q':query, 'lang':'ja', 'locale':'ja'};
    if (max_id) {
        params['max_id'] = max_id;
    }
    else {
