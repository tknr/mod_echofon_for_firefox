...
Echofon.prototype = {
  ...
  getTimeline: function(param) {
    ...
    function() {
      ...
      // return only first 20 tweets or all unread tweets to improve
		// performance
  ...
  uploadPhoto: function(msg) {
    ...
    Cu.import("resource://echofon/UpdateWithMedia.jsm");
    Cu.import("resource://echofon/TwitpicClient.jsm");
    
    switch( EchofonUtils.pref().getCharPref("imageUploadTo")) {
    case "twitter":
      var tc = new UpdateWithMedia(account, this, msg);
      tc.send(msg.images[0], msg.status);
      break;
    case "twitpic":
      var tc = new TwitpicClient(account, this, msg);
      tc.upload(msg.images[0], msg.status);
      break;
    }
  },
  ...
  deleteTweet: function(obj) {
    var account = EchofonAccountManager.instance().get();
    var req = new TwitterClient(account, this);
    req.post("statuses.destroy." + obj.id, {'trim_user': true}, "statuses_destroy");
  },
  ...
  removeAccount: function(param) {
    ...
    MuteWord.instance().removeAccount(param.user_id);
  },
  ...
  loadConfiguration: function(account) {
